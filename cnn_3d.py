...
class ConvolutionalNetwork:
    def __init__(self, image_size_x, image_size_y, num_channels):
        self.image_size_x = image_size_x
        self.image_size_y = image_size_y
        self.num_channels = num_channels  # 1 if grayscale
    ...
    def reformat(self, dataset):
        ...
        """
        return dataset.reshape((-1, self.image_size_x, self.image_size_y, self.num_channels)).astype(np.float32)
    ...
    def train(self):
        ...
        with graph.as_default():
            # Input data.
            tf_train_dataset = tf.placeholder(tf.float32, shape=(batch_size, self.image_size_x, self.image_size_y, self.num_channels))
            ...
            # Variables.
            # Layer 1 (conv)
            weights1 = weight_var([patch_size, patch_size, self.num_channels, out_features1])
            biases1 = bias_var([out_features1])
            # Layer 2 (conv)
            weights2 = weight_var([patch_size, patch_size, out_features1, out_features2])
            biases2 = bias_var([out_features2])
            # Layer 3 (fully connected)
            weights3 = weight_var([self.image_size_x // 4 * self.image_size_y // 4 * out_features2, num_hidden])
            ...
            def model(data, dropout=True):
                # Layer 1 (conv): data = `patch_size` x `patch_size` x `num_channels`
                print "conv1.shape (before conv)", data.get_shape()
                conv1 = conv2d(data, weights1)  # conv = `patch_size` x `patch_size` x `out_features1`
                ...
                conv2 = conv2d(conv1, weights2)  # conv = `patch_size` x `patch_size` x `out_features2`
                conv2 = tf.nn.relu(conv2 + biases2)
                print "conv2.shape (after conv)", conv2.get_shape()
                conv2 = max_pool_kxk(conv2, 2)  # pool = `patch_size`/2/2 x `patch_size`/2/2 x `out_features2`
                ...
                reshape = tf.reshape(conv2, [-1, shape[1] * shape[2] * shape[3]])  # reshape to fit dense layer input
